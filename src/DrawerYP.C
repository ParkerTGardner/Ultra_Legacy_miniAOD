...
void Drawer()
{
    ...
    TFile *f = new TFile("ALL_trk1_pt1_90.root");
...
*/
    ...
    c2->Divide(8);
/*
    TCanvas* c3          = new TCanvas("c3","c3", scale, scale);
    c3->Divide(ptbin,trackbin);


    TCanvas* c4          = new TCanvas("c4","c4", scale, scale);
    c4->Divide(ptbin,trackbin);
    TCanvas* c5          = new TCanvas("c5","c5", scale, scale);
    c5->Divide(ptbin,trackbin);
    TCanvas* c6          = new TCanvas("c6","c6", scale, scale);
...
*/
    //for(int wtrk =1; wtrk < trackbin+1; wtrk++){
        //for(int wppt =1; wppt <ptbin+1; wppt++){
            ...
            h1 = (TH2D*)f->Get("hSig_1_1");
            h2 = (TH2D*)f->Get("hBck_1_1");
            //h1 = (TH2D*)f->Get(Form("hSig_%d_%d",wtrk,wppt));
            //h2 = (TH2D*)f->Get(Form("hBck_%d_%d",wtrk,wppt));
            ...
            h1 = (TH2D*)f->Get(Form("hSig_2_%d",wppt));
            h2 = (TH2D*)f->Get(Form("hBck_2_%d",wppt));

            TH2D* hb1 = (TH2D*)f->Get(Form("hSig_3_%d",wppt));
            TH2D* hb2 = (TH2D*)f->Get(Form("hBck_3_%d",wppt));

            //TH2D* hc1 = (TH2D*)f->Get(Form("hSig_4_%d",wppt));
            //TH2D* hc2 = (TH2D*)f->Get(Form("hBck_4_%d",wppt));
            ...
            */
            ...
            TH1D *histfit1 = (TH1D*) h1->ProjectionY("",i,27)->Clone();
            TH1D *histfit2 = (TH1D*) h2->ProjectionY("",i,27)->Clone();
