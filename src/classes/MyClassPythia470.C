...
void MyClass::Loop(int job){
    ...
    TH1D * pthat = new TH1D("pthat",";#hat{q};#sigma (pb)",i40,i470,i3500);
    TH1D * leadingJetPt = new TH1D("leadingJetPt",";Leading p_{T}^{gen};#sigma (pb)",i50,i500,i1500);
    TH1D * jetPt = new TH1D("JetPt",";p_{T}^{gen};#sigma (pb)",i50,i100,i1500);
        ...
        for (Long64_t ievent=0; ievent <nentries; ievent ++){
            ...
            for(int ijet=0; ijet<jetN; ijet++){
                ...
                for(long int  XXtrk=0; XXtrk < NNtrk; XXtrk++ ){
                    ...
                    if(XXtrk == NNtrk - 1){ 
                        hLeadJTany  ->Fill(LeadingJT, genWeightPy);
                        hLeadJTany2d->Fill(LeadingJT,NNtrk, genWeightPy);
                    }
                ...
                for(long int  XXtrk=0; XXtrk < NNtrk; XXtrk++ ){
                    ...
                    if((*dau_chg)[ijet][XXtrk] == 0 && XXtrk == NNtrk - 1){
                        hLeadJTchg  ->Fill(LeadingJT, genWeightPy);
                        hLeadJTchg2d->Fill(LeadingJT,Nmult, genWeightPy);
                    }
                    if((*dau_chg)[ijet][XXtrk] == 0) continue;
                ...
                if(Nmult < trackbinbounds[0]){ 
                    hMult_Fail->Fill(Nmult, genWeightPy);
                    continue;
                }
                if( !F_jetpass(jetEta, jetPt, ijet, jetPtCut)){
                    hJet_Fail->Fill(Nmult, genWeightPy);
                    continue;
                }
                ...
                for(int i = 0; i < trackbin; i++){
                    if(Nmult >= trackbinbounds[i]){
                        ...
                        hN_ChgDAUvsJPT[i]   ->Fill(Nmult, (  *jetPt)[ijet], genWeightPy);
                        hN_ChgDAU[i]        ->Fill(Nmult, genWeightPy);
                    }
                    //cout << tkBool[i] << ", ";
                }
                //cout << " " << endl;
                    ...
                    for(int i = 0; i < trackbin; i++){
                        for(int j = 0; j < ptbin; j++){
                            if(tkBool[i] + A_ptBool[A_trk][j] == 2){
                                ...
                                hdNdEvsPT               [i][j]->Fill(jet_dau_eta, jet_dau_pti, genWeightPy);
