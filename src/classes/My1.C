...
const int   trackbinbounds[trackbin]    = {60,85,86,87,88,89,90};
...
const float ptbinbounds_hi[ptbin]       = {1000.0, 3.0, 3.0, 3.0, 3.0};
const float ptbinmin                    = 0.0;
const float ptbinmax                    = 1000.0;
...
bool F_eventpass(std::vector< float > *jetPt, int jetN, float jetPtCut){
...
void MyClass::Loop(int job){
    ...
    TH1D* hTotal_ijet   = new TH1D("hTotal_ijet","hTotal_ijet", trackbin+1,-LB1,trackbin);
    TH1D* hExtra_ijet   = new TH1D("hExtra_ijet","hExtra_ijet", bin5,-LB1,HB4);
    TH1D* hMissing_ijet = new TH1D("hMissing_ijet","hMissing_ijet", trackbin+1,-LB1,trackbin);


    TH1D* hEvent_Pass   = new TH1D("hEvent_Pass","hEvent_Pass", trackbin+1,-LB1,trackbin);
    TH1D* hEvent_Fail   = new TH1D("hEvent_Fail","hEvent_Fail", trackbin+1,-LB1,trackbin);
    TH1D* hJet_Pass     = new TH1D("hJet_Pass"  ,"hJet_Pass"  , trackbin+1,-LB1,trackbin);
    ...
    for(int f = 0; f<fileList.size(); f++){
        ...
        for (Long64_t ievent=0; ievent <nentries; ievent ++){
            ...
            if(jetNix == 0){hMissing_ijet->Fill(1);}
...
//                }
                ...
                h2ChgMult_v_branch->Fill(ChgMult, (*chargedMultiplicity)[ijet]); 
                h1ChgMult_d_branch->Fill(ChgMult - (*chargedMultiplicity)[ijet]); 
    ...
    TFile* fS_tempA = new TFile(Form("src/Dists/MC_Full470_F/test/Primary/Primary_job%d.root",job), "recreate");
    ...
    h2ChgMult_v_branch ->Write();
    h1ChgMult_d_branch ->Write();
    ...
    TFile* fS_tempB = new TFile(Form("src/Dists/MC_Full470_F/test/Secondary/Secondary_job%d.root",job), "recreate");
    for(int wtrk =1; wtrk <trackbin+1; wtrk++){
