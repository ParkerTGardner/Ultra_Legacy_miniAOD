...
void MyClass::Loop(int job){
    ...
    for(int f = 0; f<fileList.size(); f++){
        ...
        for (Long64_t ievent=0; ievent <nentries; ievent ++){
            ...
            std::cout << "***" << std:endl;
            std::cout << "***" << std:endl;
            std::cout << genNix << "genNix" << std:endl;
            std::cout << jetNix << "jetNix" << std:endl;
            continue;
            //chargedMultiplicity

            hEvent_Pass->Fill(1);

            h2GenN_JetN->Fill(genN, jetN);
            h2GenN_JetN_W->Fill(genN, jetN                                                  , genWeightPy);

            h1JetN_d_GenN->Fill(jetN - genN);
            h1JetN_d_GenN_W->Fill(jetN - genN                                               , genWeightPy);

            h1GenN->Fill(genN);
            h1GenN_W->Fill(genN                                                             , genWeightPy);

            h1JetN->Fill(jetN);
            h1JetN_W->Fill(jetN                                                             , genWeightPy);

            if(jetN > 1){
                h2_J1M_J2M_any->Fill((*jetNumDaughters)[0],(*jetNumDaughters)[1]            , genWeightPy);
                h2_J1M_J2M_chg->Fill((*chargedMultiplicity)[0],(*chargedMultiplicity)[1]    , genWeightPy);

                h1_J1E_d_J2E->Fill((*jetEta)[0]-(*jetEta)[1]                                , genWeightPy);
                h1_J1P_d_J2P->Fill((*jetPhi)[0]-(*jetPhi)[1]                                , genWeightPy);
            }
            if(genN > 1){ 
                h2_gen_J1M_J2M_any->Fill(genDau_chg->at(0).size(),genDau_chg->at(1).size()  , genWeightPy);
                h2_gen_J1M_J2M_chg->Fill((*genJetChargedMultiplicity)[0], (*genJetChargedMultiplicity)[1], genWeightPy);

                h1_gJ1E_d_gJ2E->Fill((*genJetEta)[0]-(*genJetEta)[1]                        , genWeightPy);
                h1_gJ1P_d_gJ2P->Fill((*genJetPhi)[0]-(*genJetPhi)[1]                        , genWeightPy);
            }
            if(jetN == 1){
                h1_single_J1M_any->Fill((*jetNumDaughters)[0]                               , genWeightPy);
                h1_single_J1M_chg->Fill((*chargedMultiplicity)[0]                           , genWeightPy);
                h1_single_J1pt->Fill((*jetPt)[0]                                            , genWeightPy);
            }
            if(jetN >= 2){
                h1_double_J1M_any->Fill((*jetNumDaughters)[0]                               , genWeightPy);
                h1_double_J1M_chg->Fill((*chargedMultiplicity)[0]                           , genWeightPy);
                h1_double_J1pt->Fill((*jetPt)[0]                                            , genWeightPy);

                h1_double_J2M_any->Fill((*jetNumDaughters)[1]                               , genWeightPy);
                h1_double_J2M_chg->Fill((*chargedMultiplicity)[1]                           , genWeightPy);
                h1_double_J2pt->Fill((*jetPt)[1]                                            , genWeightPy);
            }
            ...
            for(int ijet=0; ijet<jetN; ijet++){
                ...
                if( !F_jetpass(jetEta, jetPt, ijet, jetPtCut)){
                    hJet_Fail->Fill(ChgMult, genWeightPy);
                    continue;
                }
