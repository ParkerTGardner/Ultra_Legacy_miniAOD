...
//                        if(T_jet_dau_eta >  3.90) continue;
    ...
    //WRITING FILES FOLD
        ...
        TFile* fS_tempA = new TFile(Form("src/MC_Herwig/Primary/Primary_job%d.root",job), "recreate");
        for(int wtrk =1; wtrk <trackbin+1; wtrk++){
            for(int wppt =1; wppt <ptbin+1; wppt++){
    ...
    fS_tempA->Close();
        ...
        TFile* fS_tempB = new TFile(Form("src/MC_Herwig/Secondary/Secondary_job%d.root",job), "recreate");
        for(int wtrk =1; wtrk <trackbin+1; wtrk++){
