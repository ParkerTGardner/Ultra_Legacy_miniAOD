...
void ShiftCompareRG()
{
    TFile *f1 = new TFile("Herwig_Primary_Reco.root");
    TFile *f2 = new TFile("Herwig_Primary_Gen.root");
    ...
    for(int i = 0; i < trackbinC; i++){
        cA[i] = new TCanvas(Form("cA_%d",i),Form("Shifted Bin Comparison Reco and Gen_%d",i), Xscale, Yscale);
    }
    ...
    for(int wtrk = 0; wtrk < trackbinC; wtrk++){
        ...
        hSS1->Scale(1/(hJ1_f1->GetBinContent(wtrk+1)));
        hSS1->Scale(1./(BW1));
        ...
        hSS2->Scale(1/(hJ1_f2->GetBinContent(wtrk+1)));
        hSS2->Scale(1./(BW1));
        ...
        cA[wtrk]->cd(2);
        hSS1->Divide(hBS1);
        hSS1->SetStats(kFALSE);
        if(wtrk > 0){
        hSS1->GetXaxis()->SetRangeUser(-3,3);
        }
        hSS1->Draw("FB BB SURF1");

        cA[wtrk]->cd(3);
        hSS2->Divide(hBS2);
        hSS2->SetStats(kFALSE);
        if(wtrk > 0){
        hSS2->GetXaxis()->SetRangeUser(-3,3);
        }
        hSS2->Draw("FB BB SURF1");
