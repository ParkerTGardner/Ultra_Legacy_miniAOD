...
//P_highM_correct.C
    ...
    const int ptbin 	= 1;
    const int   trackbinbounds[trackbin]         = {20};
    //const int   trackbinboundsUpper[trackbin]    = {30};
    ...
    for(int wtrk = 0; wtrk < 1; wtrk++){
        ...
        c1_1->SetTheta(60.839);
        c1_1->SetPhi(38.0172); 
        h1->Scale(h2->GetMaximum());
        h1->GetXaxis()->SetRangeUser(-4,4);
        float h1low = .9*(h1->GetMinimum());
        float h1hi = .5*(h1->GetMaximum() - h1->GetMinimum()) + (h1->GetMinimum()) ;
        h1->GetZaxis()->SetRangeUser(h1low,h1hi);
        h1->SetTitle("");
        h1->Draw("FB BB SURF1");
        c1_1->SetLeftMargin(0.2);
        c1_1->SetTheta(60.839);
        ...
        c2_1->SetTheta(60.839);
        c2_1->SetPhi(38.0172);
        h3->GetXaxis()->SetRangeUser(-4,4);
        float h3low = .9*(h3->GetMinimum());
        float h3hi = .5*(h3->GetMaximum() - h3->GetMinimum()) + (h3->GetMinimum()) ;
        h3->GetZaxis()->SetRangeUser(h3low,h3hi);
        h3->SetTitle("");
        h3->Draw("SURF1 FB BB");
        c2_1->SetLeftMargin(0.2);
        c2_1->SetTheta(60.839);
